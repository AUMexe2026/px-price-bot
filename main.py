...
# Пороги для резких скачков (можно менять)
PUMP_THRESHOLD = 30.0     # +30% за час = срочное уведомление
DUMP_THRESHOLD = -20.0    # -20% за час = срочное уведомление
# ────────────────────────────────────────────
...
def get_price():
    try:
        ...
        if data.get("status", {}).get("error_code", 0) != 0:
            print("Ошибка CoinMarketCap:", data["status"].get("error_message"))
...
change_1h_str = f"{change_1h:+.2f}%" if change_1h is not None else "?"
change_24h_str = f"{change_24h:+.2f}%" if change_24h is not None else "?"

regular_text = f"{price_display}   [1ч: {change_1h_str}] [24ч: {change_24h_str}]"

# Проверяем резкий скачок по данным за 1 час
if change_1h is not None:
    if change_1h >= PUMP_THRESHOLD:
        send_message(f"РЕЗКИЙ ПАМП +{change_1h:.1f}% за час!\n{regular_text}", urgent=True)
    elif change_1h <= DUMP_THRESHOLD:
        send_message(f"РЕЗКИЙ ДАМП {change_1h:.1f}% за час!\n{regular_text}", urgent=True)
    else:
